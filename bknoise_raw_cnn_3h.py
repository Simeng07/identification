...
def init_params(options):
    ...
    param_init_cnn(options, params, prefix='c1',initype='fft')
    ...
    #params['h4_W'], params['h4_b'] = cnn_utils.ini_hidden_params(options['h4_option'])
    #params['h5_W'], params['h5_b'] = cnn_utils.ini_hidden_params(options['h5_option'])
    #params['h6_W'], params['h6_b'] = cnn_utils.ini_hidden_params(options['h6_option'])
    # classifier
...
def param_init_cnn(options, params, prefix='cnn', initype='random'):
    ...
    if initype=='wavlet':
        W = numpy.asarray(numpy.zeros(filter_shape), dtype=theano.config.floatX)
        ti = cnn_utils.ini_wavlet_like_filter(W)
    if initype=='fft':
        W = numpy.asarray(numpy.zeros(filter_shape), dtype=theano.config.floatX)
        ti = cnn_utils.ini_fft_like_filter(W)
...
def cnn_layer(tparams, input, options, prefix='cnn'):
    ...
    #const_filter = consider_constant(tparams[_p(prefix, 'W')])
...
def build_model(tparams, options):
    ...
    nsample = x.shape[0]
    rx = x.reshape((nsample, 1, options['c1_image'][2], 1))
    ...
    proj = tensor.dot(cnnout, tparams['h1_W']) + tparams['h1_b']   
...
def train_cnn(
    ...
    max_epochs=3000,  # The maximum number of epoch to run
    ...
    batch_size=500,  # The batch size during training.
    image_width = 32, image_height=32,
    h1_option = (39*100,500, 1),
    h2_option = (500,500, 1),   
    h3_option = (500,500, 1),
    
    c1_filter = (100, 1, 197, 1), c1_pool = (100,1), c1_image = (None,1,4096,1),
