...
def init_params(options):
    ...
    params['h3_W_1'], params['h3_b_1'] = cnn_utils.ini_hidden_params(options['h3_option'])
    ...
    params['h3_W_2'], params['h3_b_2'] = cnn_utils.ini_hidden_params(options['h3_option']) 
    # classifier
    params['U'] = 0.01 * numpy.random.randn(options['h3_option'][1]*2,
...
def build_model(tparams, options):
    ...
    x1 = x[:,0:nfeature]
    x2 = x[:,nfeature:]
    proj = tensor.dot(x1, tparams['h1_W_1']) + tparams['h1_b_1']   
    proj = tensor.tanh(proj) 
    if options['use_dropout']:
        proj = dropout_layer(proj, use_noise, trng)
    proj = tensor.dot(proj, tparams['h2_W_1']) + tparams['h2_b_1']   
    proj = tensor.tanh(proj)  
    if options['use_dropout']:
        proj = dropout_layer(proj, use_noise, trng)
    proj = tensor.dot(proj, tparams['h3_W_1']) + tparams['h3_b_1']   
    proj1 = tensor.tanh(proj)  
    if options['use_dropout']:
        proj1 = dropout_layer(proj1, use_noise, trng)
    
    proj = tensor.dot(x2, tparams['h1_W_2']) + tparams['h1_b_2']   
    proj = tensor.tanh(proj) 
    if options['use_dropout']:
        proj = dropout_layer(proj, use_noise, trng)
    proj = tensor.dot(proj, tparams['h2_W_2']) + tparams['h2_b_2']   
    proj = tensor.tanh(proj)  
    if options['use_dropout']:
        proj = dropout_layer(proj, use_noise, trng)
    proj = tensor.dot(proj, tparams['h3_W_2']) + tparams['h3_b_2']   
    proj2 = tensor.tanh(proj)  
    if options['use_dropout']:
        proj2 = dropout_layer(proj2, use_noise, trng)
     
    # hopefully, this produce (max_len,n_samples,y_dim)
    proj = tensor.concatenate([proj1,proj2], axis=1)
...
def train_cnn(
    ...
    dataset='bkclass_boost_2_2048_9.pkl.gz',
):
    ...
    if decay_c > 0.:  # 这里只是对最后一层进行decay
        ...
        weight_decay += (tparams['h1_W_1'] ** 2).sum()
        weight_decay += (tparams['h2_W_1'] ** 2).sum()
        weight_decay += (tparams['h1_W_2'] ** 2).sum()
        weight_decay += (tparams['h2_W_2'] ** 2).sum()
